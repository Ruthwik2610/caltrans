...
if app_option != "Select the Usecase":
    ...
    if app_option == "Langchain":
        ...
        with col22:
            st.subheader("Upload Revenue Data (Excel)")
        with col24:
            revenue_excel = st.file_uploader(
                "Upload Firm Revenues (Firms sheet containing Firm Name and Five Year Average)",
                type=["xlsx", "xls"],
                key="revenue_upload",
            )
        ...
        with st.sidebar:
            ...
            # --- Section 1: Institutional Memory Header ---
            st.markdown("""
            <div style="
                ...
                margin-bottom: 16px;
            ">
                <p style="margin: 0; color: white; font-size: 1rem; font-weight: 600; letter-spacing: 0.04em; text-transform: uppercase;">Institutional Memory</p>
            </div>
            """, unsafe_allow_html=True)
            
            st.markdown('<p style="color: rgba(255,255,255,0.75); font-size: 0.82rem; margin: 0 0 16px 0; line-height: 1.5;">The AI learns from your corrections. When the database is full, consolidate.</p>', unsafe_allow_html=True)
            ...
            # --- Section 2: Metric Card with Progress Bar ---
            st.markdown(f"""
            <div style="
            ...
            ">
                <div style="display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 8px;">
                    <p style="margin: 0; color: rgba(255,255,255,0.7); font-size: 0.78rem; font-weight: 500; letter-spacing: 0.03em; text-transform: uppercase;">Active Corrections</p>
                    <p style="margin: 0; color: rgba(255,255,255,0.5); font-size: 0.72rem;">45/level</p>
                </div>
                <p style="margin: 0 0 10px 0; color: white; font-size: 1.8rem; font-weight: 700; letter-spacing: -0.02em;">{total_precedents}<span style="color: rgba(255,255,255,0.4); font-size: 1rem; font-weight: 400;"> / 135</span></p>
                <div style="background: rgba(255,255,255,0.15); border-radius: 6px; height: 6px; overflow: hidden;">
                    <div style="background: linear-gradient(90deg, #60a5fa, #a78bfa); height: 100%; width: {pct}%; border-radius: 6px; transition: width 0.5s ease;"></div>
                </div>
            </div>
            """, unsafe_allow_html=True)
            
            # --- Section 3: Consolidate Header ---
            st.markdown("""
            <div style="
                background: rgba(255,255,255,0.12);
                backdrop-filter: blur(10px);
                ...
                margin-bottom: 12px;
            ">
                <p style="margin: 0; color: white; font-size: 0.95rem; font-weight: 600; letter-spacing: 0.04em; text-transform: uppercase;">Consolidate Corrections</p>
            </div>
            """, unsafe_allow_html=True)
            
            st.markdown('<p style="color: rgba(255,255,255,0.75); font-size: 0.82rem; margin: 0 0 12px 0; line-height: 1.5;">Merge corrections into a clean <code style="background: rgba(255,255,255,0.15); padding: 2px 6px; border-radius: 4px; color: rgba(255,255,255,0.9); font-size: 0.78rem;">rules.json</code> file.</p>', unsafe_allow_html=True)
            if total_precedents < 15:
                ...
                ">
                    <p style="margin: 0; color: rgba(255,255,255,0.65); font-size: 0.78rem; line-height: 1.5;">
                        Collect at least <strong style="color: rgba(255,255,255,0.9);">15 corrections</strong> before consolidating for best results.
                    </p>
                </div>
                """, unsafe_allow_html=True)
            
            if st.button("Generate Clean Rules", use_container_width=True):
                with st.spinner("AI is synthesizing database..."):
            ...
            if st.session_state.get('show_consolidation_success') and 'consolidated_rules_json' in st.session_state:
                ...
                <div style="
                    ...
                    margin: 10px 0;
                ">
                    <p style="margin: 0; color: rgba(255,255,255,0.9); font-size: 0.82rem;">&#x2705; Consolidation complete. Download your rules below.</p>
                </div>
                """, unsafe_allow_html=True)
                st.download_button(
        ...
        if cucp_files:
            ...
            if st.session_state.eval_stage == 0:
                ...
                uploaded_rules = st.file_uploader("Optional: Upload an existing `rules.json` file to inherit previous memory before starting.", type=['json'])
                if uploaded_rules is not None:
                    try:
                        import json
                        rules_dict = json.load(uploaded_rules)
                        overwrite_db(rules_dict)
                        st.success("Successfully loaded institutional memory from uploaded file!")
                    except Exception as e:
                        st.error(f"Failed to load rules: {e}")
                        
                st.markdown("---")
                if st.button("Start AI Evaluation ➔", type="primary"):
                    with st.spinner(f"Running Level 1: Fact Extraction on **{file_name}**..."):
            ...
            # STATE 1: Level 1 Fact Review
                ...
                st.markdown("### Level 1: Fact Extraction Review")
                st.info("Please review the extracted facts below. If an extracted fact is incorrect or something is missing, log a correction.")
                ...
                with st.form("l1_correction_form"):
                    st.markdown("#### Log a Fact Correction")
                    
                    if l1_count >= 45:
                        st.error(f"🚨 Memory Database Limit Reached ({l1_count}/45). You cannot add new corrections to this level until existing corrections are consolidated into the main rubric via the Sidebar.")
                    elif l1_count >= 36:
                        st.warning(f"⚠️ Memory Database is nearing capacity ({l1_count}/45). Prepare to consolidate corrections soon.")
                        
                    c1, c2 = st.columns(2)
                    with c1:
                        target_fact = st.selectbox(
                            "Fact to Change (Context)", 
                            ["Firm Name", "Cross-Reference Result", "Narrative Declared PNW", "Demographic Flags Missed", "Specific Incident Detail"],
                    ...
                    reasoning_val = st.text_area(
                            "Reasoning for Change (What should the model remember?)",
                            help="Provide the correction rationale. The AI will save this in its memory and apply it to all future applications to prevent this hallucination."
                        )
                    
                    # Disable submit if over threshold
                    is_disabled = True if l1_count >= 45 else False
                    if st.form_submit_button("Stage Correction", disabled=is_disabled):
                        st.session_state.staged_precedents["level_1_precedents"].append({
                            "target": target_fact,
                            "correction": correction_val,
                            "human_reasoning": reasoning_val
                        })
                        st.success("Correction temporarily staged!")
                        st.session_state.eval_stage = 0 # Re-run Level 1 with new memory
                        st.rerun()
                
                st.caption("*Note: AI corrections are staged temporarily and only permanently saved to the global database after you approve the entire 3-Level evaluation.*")
                ...
                if st.button("Approve Level 1 & Proceed ➔", type="primary"):
                    with st.spinner("Running Level 2: Legal Classification..."):
            ...
            # STATE 2: Level 2 Classification Review
                ...
                st.markdown("### Level 2: Legal Classification Review")
                st.info("The AI has mapped the extracted facts to legal categories. Review and correct if misapplied.")
                st.caption("*Tip: Double-click any cell in the table below to expand it and read the full text.*")
                ...
                if st.button("⬅️ Go Back to Level 1"):
                    st.session_state.eval_stage = 1
                    st.rerun()
                
                l2_count = get_precedent_count(2) + len(st.session_state.staged_precedents.get("level_2_precedents", []))
                with st.form("l2_correction_form"):
                    st.markdown("#### Log a Classification Override")
                    
                    if l2_count >= 45:
                        st.error(f"🚨 Memory Database Limit Reached ({l2_count}/45). You cannot add new corrections to this level until existing corrections are consolidated into the main rubric via the Sidebar.")
                    elif l2_count >= 36:
                        st.warning(f"⚠️ Memory Database is nearing capacity ({l2_count}/45). Prepare to consolidate corrections soon.")
                        
                    c1, c2 = st.columns(2)
                    with c1:
                        target_class = st.selectbox(
                            "Scenario to Reclassify", 
                            [c.get('classification', '') + ' (Change this)' for c in classifications] + ["General Scenario Misclassification"],
                    ...
                    reasoning_class = st.text_area(
                            ...
                            help="Provide your legal justification. The AI will learn this correction and apply it to classify similar subjective facts in the future."
                        )
                    
                    is_disabled = True if l2_count >= 45 else False
                    if st.form_submit_button("Stage Override", disabled=is_disabled):
                        st.session_state.staged_precedents["level_2_precedents"].append({
                            "target": target_class,
                            "correction": correction_class,
                            "human_reasoning": reasoning_class
                        })
                        st.success("Approval override temporarily staged!")
                        # To regenerate with new instructions:
                ...
                if st.button("Approve Level 2 & Proceed ➔", type="primary"):
                    with st.spinner("Running Level 3: Evidentiary Thresholds..."):
            ...
            # STATE 3: Level 3 Threshold Review
                ...
                st.markdown("### Level 3: Evidentiary Threshold Review")
                st.info("The AI has determined Pass/Fail based on the magnitude of evidence. Adjust strictness or leniency below.")
                st.caption("*Tip: Double-click any cell in the table below to expand it and read the full text.*")
                ...
                # Show all required columns for scrolling horizontally
                cols_to_show = ['category', 'qualification', 'evidence_summary', 'reasoning', 'pass_fail', 'request_info', 'confidence']
                existing_cols = [c for c in cols_to_show if c in df_crit.columns]
                st.dataframe(df_crit[existing_cols], use_container_width=True)
                
                if st.button("⬅️ Go Back to Level 2"):
                    st.session_state.eval_stage = 2
                    st.rerun()
                ...
                with st.form("l3_correction_form"):
                    st.markdown("#### Log a Threshold Override")
                    
                    if l3_count >= 45:
                        st.error(f"🚨 Memory Database Limit Reached ({l3_count}/45). You cannot add new corrections to this level until existing corrections are consolidated into the main rubric via the Sidebar.")
                    elif l3_count >= 36:
                        st.warning(f"⚠️ Memory Database is nearing capacity ({l3_count}/45). Prepare to consolidate corrections soon.")
                        
                    target_crit = st.selectbox(
                        "Criterion or Decision to Adjust", 
                        [c.get('qualification') for c in criteria] + ["Final Decision"],
                        help="Select the specific rubric criterion where the AI was too strict or too lenient."
                    )
                    correction_crit = st.selectbox(
                        "New Result", 
                        ["Pass", "Fail", "Request Additional Information"],
                        help="Select what the threshold decision actually should be."
                    )
                    reasoning_crit = st.text_area(
                        "Standard of Proof Rationale (Why is this magnitude sufficient or insufficient?)",
                        help="Provide standard-of-proof guidance. The AI will save this legal threshold and apply it so it matches your department's exact definition of 'preponderance' in future reviews."
                    )
                    
                    is_disabled = True if l3_count >= 45 else False
                    if st.form_submit_button("Stage Threshold Correction", disabled=is_disabled):
                        st.session_state.staged_precedents["level_3_precedents"].append({
                            "target": target_crit,
                            "correction": correction_crit,
                            "human_reasoning": reasoning_crit
                        })
                        st.success("Threshold correction temporarily staged!")
                        with st.spinner("Re-evaluating Thresholds..."):
                            st.session_state.l3_data = run_level_3_thresholds(
                                st.session_state.l2_data.get("classifications", []), 
                                st.session_state.l1_data.get("extracted_facts", []), 
                                st.session_state.l1_data.get("cross_reference_result", "None"),
                                staged_precedents=st.session_state.staged_precedents.get("level_3_precedents", [])
                            )
                            st.rerun()
